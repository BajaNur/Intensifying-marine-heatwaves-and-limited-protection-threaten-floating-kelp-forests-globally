# Kelp Step 5: Extract daily time series from each grid-square and compute MHW stats and save
...
# Folders --------------------------------------------------------
  ...
  working_folder <- make_folder("/Volumes/BackPack/tmp")
	output_folder <- make_folder("Final_Cum_Int")
		...
		# Function to extract cum_MHW_impact from each time series
  		get_mhw_imp <- function(ssts, 
  		  ...
  		  if(sum(!is.na(df$SST)) > 0) {
  		    ...
  		    # Then the events
  		    yrs <- tibble(year = unique(year(as.Date(dates)))) #***be careful that this didn't break anything
