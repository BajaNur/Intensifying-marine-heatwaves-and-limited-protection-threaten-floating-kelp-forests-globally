...
# Load data --------------------------------------------------------------------
meow <- st_read("Data/Spalding Ecoregions/meow_ecos.shp") # Download here: https://databasin.org/datasets/3b6b12e7bcca419990c9081c0af254a2/
  

hist <- read_csv("Data/MHW/OISST_median.csv") %>% 
  select(-1) %>% 
  rename(lon = x, lat = y) %>% 
...
ssp126 <- read_csv("Data/MHW/ssp126_median.csv") %>% 
  select(-1) %>%
  rename(lon = x, lat = y) %>% 
...
ssp245 <- read_csv("Data/MHW/ssp245_median.csv") %>% 
  select(-1) %>%
  rename(lon = x, lat = y) %>% 
  pivot_longer(cols = c(3:121),
               names_to = "year",
...
ssp585 <- read_csv("Data/MHW/ssp585_median.csv") %>% 
  select(-1) %>%
  rename(lon = x, lat = y) %>% 
  pivot_longer(cols = c(3:121),
               names_to = "year",
...
ggsave(plot = fig,
       filename = ("img/Map_and_mhw_by_realm.pdf"),
       width = 8,
       height = 10)
