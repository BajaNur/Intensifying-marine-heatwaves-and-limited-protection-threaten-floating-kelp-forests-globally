# Kelp Step 8: Repeat Step 5, but for OISST to check whether historical model MHWs are a reasonable approximation of observations
...
# Folders --------------------------------------------------------
  ...
  input_folder <- "/Volumes/Mega_Disk/OISST_MHW"
  oisst_folder <- make_folder("/Volumes/Bullet_RAID/oisst")
  working_folder <- make_folder("/Volumes/Bullet_RAID/tmp")
	output_folder <- make_folder("OISST_Cum_Int")
...
# Regrid OISST and fill with inverse-dist-weighted means ------------------

  mask <- "/Volumes/Mega_Disk/OISST/mask_tos.nc"
	oisst <- dir(input_folder, full.names = TRUE) %>% 
	  str_subset("_MHW.nc", negate = TRUE)
	out_file <- paste0(oisst_folder, "/oisst_regrid_idw.nc")
	cdo_code <- paste0("cdo -L -setmisstodis,8 -remapbil,", mask, " ", oisst, " ", out_file)
	  system(cdo_code)
...
# Subset the world --------------------------------------------------------
  ...
  f <- dir(oisst_folder, full.names = TRUE)
  ff <- f %>% 
    str_replace(oisst_folder, working_folder)
  file.copy(f, ff)
  do_split <- function(b) {
    output_file <- f %>% 
      str_replace(oisst_folder, working_folder) %>%
...
# Do the work -------------------------------------------------------------
	  ...
	  plan(sequential)
	  out_name <- f %>% 
	    str_replace(".nc", "_1982-2021.csv") %>%
	    str_replace("oisst_", "tos_Oday_oisst_oisst_") %>% 
	    str_replace(oisst_folder, output_folder)
	  write_csv(out, out_name)
	  terminal_code <- paste0("rm ", paste0(nc_boxes, collapse = " "))
	    system(terminal_code)
	    
# Goto Kelp_9_Compare_Historical_OISST.R
